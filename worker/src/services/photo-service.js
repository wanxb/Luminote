...
let ensurePhotoVisibilityColumnPromise = null;
function buildMockAssetUrl(origin, variant, id) {
    return `${origin}/mock-storage/${variant}/${id}`;
}
function buildAssetUrl(origin, variant, id) {
    return `${origin}/assets/${variant}/${id}`;
}
function createPhotoId(index) {
...
function extractPhotoIdFromObjectKey(key) {
    const match = key.match(/^(?:thumbs|display|display-watermarked)\/(photo_[^./]+(?:_[^./]+)?)\.(?:webp|jpg)$/);
    return match?.[1] ?? null;
}
async function listPhotosFromBucket(env, origin) {
...
async function ensurePhotoVisibilityColumn(env) {
    if (!env.DB) {
        return;
    }
    if (!ensurePhotoVisibilityColumnPromise) {
        ensurePhotoVisibilityColumnPromise = (async () => {
            try {
                await env
                    .DB.prepare("ALTER TABLE photos ADD COLUMN is_hidden INTEGER NOT NULL DEFAULT 0")
                    .run();
            }
            catch {
...
export async function listPhotos(env, origin, tag = null, options) {
    if (!env.DB) {
...
export async function getPhotoById(env, origin, id, options) {
    if (!env.DB) {
...
export async function createPhotos(env, origin, inputs) {
    ...
    const storageResults = await Promise.all(created.map((photo, index) => storePhotoObjects(env, {
        id: photo.id,
    ...
    const statements = created.map((photo, index) => {
        ...
        created_at
            ...
            .bind(photo.id, input.fileName, "", input.description, storage.originalKey ? storage.originalKey : displayUrl, thumbUrl, displayUrl, watermarkedDisplayUrl, input.exif?.takenAt ?? null, input.exif?.device ?? null, input.exif?.lens ?? null, input.exif?.location ?? null, input.exif?.exif ? JSON.stringify(input.exif.exif) : null, JSON.stringify(input.tags), 0, input.showCameraInfo ? 1 : 0, input.showDateInfo ? 1 : 0, input.showLocationInfo ? 1 : 0, input.watermarkEnabled ? 1 : 0, createdAt);
    });
    await env.DB.batch(statements);
    return created;
}
export async function deletePhotoById(env, id) {
    if (!env.DB) {
        return {
            ok: false,
            deleted: false,
    ...
    const existing = await env.DB.prepare(`SELECT id FROM photos WHERE id = ? LIMIT 1`)
        .bind(id)
        .first();
    if (!existing) {
        return {
...
export async function updatePhotoById(env, id, payload) {
    if (!env.DB) {
        return {
            ok: false,
            persisted: false,
