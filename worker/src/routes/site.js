...
function validateTextField(value, maxLength, emptyMessage, tooLongMessage, options) {
    const trimmed = value.trim();
    if (!options?.allowEmpty && trimmed.length === 0) {
        return { ok: false, error: emptyMessage };
    }
    if (!isWithinTextLimit(trimmed, maxLength)) {
        return { ok: false, error: tooLongMessage };
    }
    ...
    try {
        ...
        if (body.siteTitle !== undefined) {
            if (typeof body.siteTitle !== "string" ||
                ...
                const result = validateTextField(body.siteTitle, TEXT_LIMITS.siteTitle, "站点标题不能为空。", `站点标题不能超过 ${TEXT_LIMITS.siteTitle} 个字符。`);
                if (!result.ok) {
        ...
        if (body.siteDescription !== undefined) {
            if (typeof body.siteDescription !== "string") {
                ...
                const result = validateTextField(body.siteDescription, TEXT_LIMITS.siteDescription, "站点简介格式错误。", `站点简介不能超过 ${TEXT_LIMITS.siteDescription} 个字符。`, { allowEmpty: true });
                if (!result.ok) {
                    errors.push(result.error);
                }
                else {
        ...
        if (body.watermarkText !== undefined) {
            if (typeof body.watermarkText !== "string" ||
                ...
                const result = validateTextField(body.watermarkText, TEXT_LIMITS.watermarkText, "水印文本不能为空。", `水印文本不能超过 ${TEXT_LIMITS.watermarkText} 个字符。`);
                if (!result.ok) {
                    errors.push(result.error);
                }
                else {
        ...
        if (body.maxTagsPerPhoto !== undefined) {
            if (!Number.isInteger(body.maxTagsPerPhoto) ||
        ...
        if (body.photographerAvatarUrl !== undefined) {
            if (typeof body.photographerAvatarUrl !== "string") {
                ...
                const result = validateTextField(body.photographerAvatarUrl, TEXT_LIMITS.url, "摄影师头像链接格式错误。", `摄影师头像链接不能超过 ${TEXT_LIMITS.url} 个字符。`, { allowEmpty: true });
                if (!result.ok) {
                    errors.push(result.error);
                }
                else {
        ...
        if (body.photographerName !== undefined) {
            if (typeof body.photographerName !== "string") {
                ...
                const result = validateTextField(body.photographerName, TEXT_LIMITS.photographerName, "摄影师姓名格式错误。", `摄影师姓名不能超过 ${TEXT_LIMITS.photographerName} 个字符。`, { allowEmpty: true });
                if (!result.ok) {
                    errors.push(result.error);
                }
                else {
        ...
        if (body.photographerBio !== undefined) {
            if (typeof body.photographerBio !== "string") {
                ...
                const result = validateTextField(body.photographerBio, TEXT_LIMITS.photographerBio, "摄影师简介格式错误。", `摄影师简介不能超过 ${TEXT_LIMITS.photographerBio} 个字符。`, { allowEmpty: true });
                if (!result.ok) {
                    errors.push(result.error);
                }
                else {
        ...
        if (body.photographerEmail !== undefined) {
            if (typeof body.photographerEmail !== "string") {
                ...
                const result = validateTextField(body.photographerEmail, TEXT_LIMITS.email, "邮箱格式错误。", `邮箱不能超过 ${TEXT_LIMITS.email} 个字符。`, { allowEmpty: true });
                if (!result.ok) {
                    errors.push(result.error);
                }
                else {
        ...
        if (body.photographerXiaohongshu !== undefined) {
            if (typeof body.photographerXiaohongshu !== "string") {
                ...
                const result = validateTextField(body.photographerXiaohongshu, TEXT_LIMITS.accountName, "小红书账号格式错误。", `小红书账号不能超过 ${TEXT_LIMITS.accountName} 个字符。`, { allowEmpty: true });
                if (!result.ok) {
                    errors.push(result.error);
                }
                else {
        ...
        if (body.photographerXiaohongshuUrl !== undefined) {
            if (typeof body.photographerXiaohongshuUrl !== "string") {
                ...
                const result = validateTextField(body.photographerXiaohongshuUrl, TEXT_LIMITS.url, "小红书链接格式错误。", `小红书链接不能超过 ${TEXT_LIMITS.url} 个字符。`, { allowEmpty: true });
                if (!result.ok) {
                    errors.push(result.error);
                }
                else {
        ...
        if (body.photographerDouyin !== undefined) {
            if (typeof body.photographerDouyin !== "string") {
                ...
                const result = validateTextField(body.photographerDouyin, TEXT_LIMITS.accountName, "抖音账号格式错误。", `抖音账号不能超过 ${TEXT_LIMITS.accountName} 个字符。`, { allowEmpty: true });
                if (!result.ok) {
                    errors.push(result.error);
                }
                else {
        ...
        if (body.photographerDouyinUrl !== undefined) {
            if (typeof body.photographerDouyinUrl !== "string") {
                ...
                const result = validateTextField(body.photographerDouyinUrl, TEXT_LIMITS.url, "抖音链接格式错误。", `抖音链接不能超过 ${TEXT_LIMITS.url} 个字符。`, { allowEmpty: true });
                if (!result.ok) {
                    errors.push(result.error);
                }
                else {
        ...
        if (body.photographerInstagram !== undefined) {
            if (typeof body.photographerInstagram !== "string") {
                ...
                const result = validateTextField(body.photographerInstagram, TEXT_LIMITS.accountName, "Instagram 账号格式错误。", `Instagram 账号不能超过 ${TEXT_LIMITS.accountName} 个字符。`, { allowEmpty: true });
                if (!result.ok) {
                    errors.push(result.error);
                }
                else {
        ...
        if (body.photographerInstagramUrl !== undefined) {
            if (typeof body.photographerInstagramUrl !== "string") {
                ...
                const result = validateTextField(body.photographerInstagramUrl, TEXT_LIMITS.url, "Instagram 链接格式错误。", `Instagram 链接不能超过 ${TEXT_LIMITS.url} 个字符。`, { allowEmpty: true });
                if (!result.ok) {
                    errors.push(result.error);
                }
                else {
        ...
        if (body.photographerCustomAccount !== undefined) {
            if (typeof body.photographerCustomAccount !== "string") {
                ...
                const result = validateTextField(body.photographerCustomAccount, TEXT_LIMITS.accountName, "自定义账号名称格式错误。", `自定义账号名称不能超过 ${TEXT_LIMITS.accountName} 个字符。`, { allowEmpty: true });
                if (!result.ok) {
                    errors.push(result.error);
                }
                else {
        ...
        if (body.photographerCustomAccountUrl !== undefined) {
            if (typeof body.photographerCustomAccountUrl !== "string") {
                ...
                const result = validateTextField(body.photographerCustomAccountUrl, TEXT_LIMITS.url, "自定义账号链接格式错误。", `自定义账号链接不能超过 ${TEXT_LIMITS.url} 个字符。`, { allowEmpty: true });
                if (!result.ok) {
                    errors.push(result.error);
                }
                else {
