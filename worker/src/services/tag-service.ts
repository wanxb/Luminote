...
const DEFAULT_TAGS = ["人像", "动物", "植物", "建筑", "街拍", "鸟类", "黑白", "夜景"];
...
async function ensureDefaultTags(env: Env) {
  ...
  const countResult = await env.DB.prepare("SELECT COUNT(*) as count FROM tag_pool").first<{ count: number }>();
  ...
  const statements = DEFAULT_TAGS.map((name, index) =>
    env.DB!.prepare("INSERT INTO tag_pool (id, name, created_at) VALUES (?, ?, ?)").bind(
...
export async function createTag(env: Env, name: string, maxTagPoolSize: number) {
  if (!env.DB) {
    return null;
  }
...
export async function deleteTag(env: Env, id: string) {
  ...
  try {
    const existing = await env.DB.prepare("SELECT id FROM tag_pool WHERE id = ? LIMIT 1").bind(id).first<{ id: string }>();
    ...
    const result = await env.DB.prepare("DELETE FROM tag_pool WHERE id = ?").bind(id).run();
